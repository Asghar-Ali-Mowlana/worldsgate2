...
class _DeoManageHotelsState extends State<DeoManageHotels> {
  ...
  getyo() async {
    FirebaseFirestore.instance.collection('hotels').where('dataentryuid', isEqualTo: widget.uid).get().then((myDocuments){
  ...
  List<Widget> newbuilder() {
    ...
    for (int i = 0; i < entryList.length; i++) {
      ...
      m.add(Container(
        ...
        child: Column(
          children: [
            Text("${entryList[i].key} (${entryList[i].value.length.toString()})", style: TextStyle(
                color: Colors.white
            ),),
